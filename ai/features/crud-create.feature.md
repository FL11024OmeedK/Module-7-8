...
- Update the Create form in `AgentForm.jsx` to collect agent fields: first name, last name, email, region, rating, fee
- On submit, send a `POST` request to `POST /agents` with the form data as JSON
...
- Any backend changes — `POST /agents` is already implemented
...
- Submit handler — sends `POST` to `http://localhost:5050/agents` with JSON body, then navigates to `/`
...
5. The frontend sends `POST http://localhost:5050/agents` with the form data as JSON
...
- `client/src/components/AgentForm.jsx` — the only file that changes

### Backend / API

- `POST /agents` — receives the agent fields in `req.body`, calls `createAgent()`, inserts into `agentsDb.collection("agents")`, returns 201 (already implemented, no changes needed)
...
- Do not rename the file `AgentForm.jsx`
...
- [ ] Submitting the form sends a POST request to `http://localhost:5050/agents`
...
- `AgentForm.jsx` handles both Create and Edit in the same component — `isNew` determines which mode is active. Do not break this logic.
