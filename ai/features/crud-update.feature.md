...
- On submit, a `PATCH` request is sent to `PATCH /agents/:id` with the updated data
...
- Any new backend routes — `PATCH /agents/:id` is already implemented
...
- Data Pre-population — when the form mounts with an `id` param, it fetches the agent from `GET /agents/:id` and loads the data into the form fields
- isNew Flag — `isNew` is set to `false` when editing, which causes `onSubmit` to send a PATCH instead of a POST
- Submit Handler — sends `PATCH http://localhost:5050/agents/:id` with the updated form data as JSON
...
3. `AgentForm.jsx` mounts — `useEffect` detects the `id` param, sets `isNew` to `false`
4. A `GET` request fetches the agent data from `http://localhost:5050/agents/:id`
5. The form fields are pre-populated with the agent's current values
6. User edits one or more fields and clicks "Save Agent"
7. The frontend sends `PATCH http://localhost:5050/agents/:id` with the updated data
...
- `client/src/components/AgentForm.jsx` — handles both Create and Edit; no structural changes needed
- `client/src/components/AgentList.jsx` — Edit link already points to `/edit/${agent._id}`

### Backend / API

- `GET /agents/:id` — fetches a single agent to pre-populate the form (already implemented)
- `PATCH /agents/:id` — updates the agent document in MongoDB using `updateAgent()` (already implemented)
...
- Do not rename or split `AgentForm.jsx` — Create and Edit intentionally share this component
- The `key` prop used for routing must remain `agent._id`
...
- [ ] `PATCH /agents/:id` returns a 200 response with `modifiedCount: 1`
...
- The `useEffect` in `AgentForm.jsx` already handles pre-population: it checks for `params.id`, fetches the agent, and calls `setForm(agent)` — this will populate the new fields automatically as long as the field names in state match the field names returned by MongoDB
